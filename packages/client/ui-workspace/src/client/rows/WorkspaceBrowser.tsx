...
> & {
  ...
  /** Restrict rows to one boot workspace working directory (VS Code embed). */
  filterCwd?: string
}) {
  const panelActive = usePanelInfo(info => info.activePanelId !== null)
  const statuses = useSessionStatus(s => s)
  const rows = useMemo(
    () => {
      const derived = deriveFlat(list, sessionIds, statuses)
    ...
    [list, sessionIds, statuses, filterCwd],
  )
  const [drag, setDrag] = useState<DragState | null>(null)
...
export function WorkspaceBrowser({
  ...
  // Embedded (VS Code sidebar) mode pins the browser to the flat list.
  const effectiveGroupBy = embed ? 'flat' : groupBy
